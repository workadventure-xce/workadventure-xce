import type { ITiledMapTileLayer, ITiledMapProperty, Json } from "@workadventure/tiled-map-type-guard";
...
export class InteractiveLayer extends Container {
     ...
     */
    private getLayerProperty(name: string): Json | undefined {
        ...
        const prop = properties.find((property: ITiledMapProperty) => property.name === name);
    ...
    private addFrameToTilesetImage(tileset: Tileset, key: string, index: number): void {
        if (tileset.image && !tileset.image.has(key)) {
